...
export function HowItWorks() {
  const c = useContent("HowItWorks");

  // Support CMS: either an array of groups or fallback to default
  const steps = Array.isArray(c.groups)
    ? c.groups.map((g, i) => ({
        step: g.step || `${i + 1}`,
        icon: iconMap[g.icon as keyof typeof iconMap] || Database,
    ...
    : [
        {
          ...
          description:
            "Import your existing data or start fresh with our intuitive setup wizard. Configure your mill's specific processes and workflows.",
        },
        {
          step: "2",
          icon: Settings,
          ...
          description:
            "Let RMMS handle routine tasks like inventory tracking, billing, procurement, and vendor management automatically.",
        },
        {
          step: "3",
          icon: TrendingUp,
          ...
          description:
            "Access real-time dashboards and analytics to make informed decisions and scale your rice mill operations efficiently.",
        },
      ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center space-y-4 mb-16">
          <h2 className="text-3xl lg:text-5xl font-bold text-foreground">
            {safeText(c.headingPrefix )|| "How It"}{" "}
            <span className="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">
              {safeText(c.headingHighlight )|| "Works"}
            </span>
          </h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            {safeText(c.subheading) ||
